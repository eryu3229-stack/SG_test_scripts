...
范围口径（用户 2026-09-18 明确要求"不要考虑比较复杂的设置"）：
    只保留**单通道、单值、内部源**的基础设置 —— 每种调制一个开关、一个源、
    一个主参数（深度/偏差/周期宽度）。以下一律不做：
...
class SignalGenerator:
    ...
    def set_power(self, power):
        ...
        Returns:
            bool: 是否成功
        """
        return self._write(f"POW {power} DBM", f"设置信号源功率为: {power} dBm")
    ...
    def set_lf_shape(self, shape, channel=1):
        """设置内部 LF 调制源波形（手册 p501）

        Args:
            shape: SINE / SQUare / PULSe / TRIangle / TRAPeze
            channel: LF 通道 1|2（AM1/FM1/PM1 用 LF1）
        ...
        if value is None:
            return False
        return self._write(f"SOUR{self.PATH}:LFO{channel}:SHAP {value}",
                           f"LF{channel} 波形: {value}")

    def set_lf_frequency(self, frequency, channel=1):
        ...
        Returns:
            bool: 是否成功
        """
        return self._write(f"SOUR{self.PATH}:LFO{channel}:FREQ {frequency}",
                           f"LF{channel} 调制频率: {frequency} Hz")

    def enable_lf_output(self, enable=True, channel=1):
        """开启 / 关闭 LF 输出（手册 p499）
        ...
        Returns:
            bool: 是否成功
        """
        text = f"开启 LF{channel} 输出" if enable else f"关闭 LF{channel} 输出"
        return self._write(f"SOUR{self.PATH}:LFO{channel}:STAT {self._onoff(enable)}", text)
    ...
    def enable_am(self, enable=True, channel=1):
        ...
        Args:
            enable: True 开启、False 关闭
            channel: 调制通道 1|2（对应 AM1 / AM2）

        Returns:
            bool: 是否成功
        """
        text = f"开启 AM{channel}（幅度调制）" if enable else f"关闭 AM{channel}"
        return self._write(f"SOUR{self.PATH}:AM{channel}:STAT {self._onoff(enable)}", text)

    def set_am_source(self, source, channel=1):
        """选择幅度调制的信号源（手册 p446）
        ...
        if value is None:
            return False
        return self._write(f"SOUR{self.PATH}:AM{channel}:SOUR {value}",
                           f"AM{channel} 调制源: {value}")
    ...
    def set_am_depth(self, depth, channel=1):
        ...
        if depth is None or not (0 <= depth <= 100):
            ...
            return False
        return self._write(f"SOUR{self.PATH}:AM{channel}:DEPTH {depth}",
                           f"AM{channel} 调幅深度: {depth} %")
    ...
    def enable_fm(self, enable=True, channel=1):
        ...
        Returns:
            bool: 是否成功
        """
        text = f"开启 FM{channel}（频率调制）" if enable else f"关闭 FM{channel}"
        return self._write(f"SOUR{self.PATH}:FM{channel}:STAT {self._onoff(enable)}", text)

    def set_fm_source(self, source, channel=1):
        """选择频率调制的信号源（手册 p450）

        Args:
            source: 取值同 AM，见 set_am_source
            channel: 调制通道 1|2（*RST 默认 FM1→LF1、FM2→LF2）
        ...
        if value is None:
            return False
        return self._write(f"SOUR{self.PATH}:FM{channel}:SOUR {value}",
                           f"FM{channel} 调制源: {value}")

    def set_fm_deviation(self, deviation, channel=1):
        ...
        if deviation is None or deviation < 0:
            ...
            return False
        return self._write(f"SOUR{self.PATH}:FM{channel}:DEV {deviation}",
                           f"FM{channel} 调频偏差: {deviation} Hz")
    ...
    def enable_pm(self, enable=True, channel=1):
        ...
        **注意**：手册明确写着"激活相位调制会关闭频率调制"——
        ...
        Returns:
            bool: 是否成功
        """
        text = f"开启 PM{channel}（相位调制）" if enable else f"关闭 PM{channel}"
        return self._write(f"SOUR{self.PATH}:PM{channel}:STAT {self._onoff(enable)}", text)

    def set_pm_source(self, source, channel=1):
        """选择相位调制的信号源（手册 p455）
        ...
        if value is None:
            return False
        return self._write(f"SOUR{self.PATH}:PM{channel}:SOUR {value}",
                           f"PM{channel} 调制源: {value}")

    def set_pm_deviation(self, deviation, channel=1):
        ...
        if deviation is None or deviation < 0:
            ...
            return False
        return self._write(f"SOUR{self.PATH}:PM{channel}:DEV {deviation} RAD",
                           f"PM{channel} 相位偏差: {deviation} rad")
